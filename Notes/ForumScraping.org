...
#+BEGIN_SRC python
  import requests
  r = requests.get('http://info.cern.ch/hypertext/DataSources/Top.html')
#+END_SRC

This snippet of code submits an [[https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol#Request_methods][HTTP GET request]] to the website http://info.cern.ch/hypertext/DataSources/Top.html (which I've picked for its simplicity) and gets its HTML code along with some other data and stores it in a requests object. To check if the above snippet was successful we can check the request object's status code.
